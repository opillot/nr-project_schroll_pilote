CREATE PROCEDURE [SYNCH].[sp_synchroDataDelete] 
...
BEGIN
	...
	Declare @testfield as VARCHAR(50) = '';
	...
	Declare @QUERYDelete as NVARCHAR(MAX) = '';

	Set @QUERYDelete = 'Update ' + @reftable +
			' Set ' + @reftable + ' .[Activ] = 0 ' + 
			' From ' + @synchtable + ' ' + @aliassynchtable +
			' Left Outer Join ' + @reftable + 
			' On ' + @joinfields + 
			' Where ' + @aliassynchtable + '.[' + @testfield + '] is null';
	Select 'Processing ' + @reftable + ' : ' + @QUERYDelete;
	--exec sp_executesql @QUERYDelete;
END